...
var ddpowerzoomer = {
  ...
  mousewheelevt: /Firefox/i.test(navigator.userAgent)
    ? "DOMMouseScroll"
  ...
  movemagnifier: function (e, moveBol, zoomdir) {
    ...
    if (moveBol == true) {
      ...
      //move magnifier so it follows the cursor
        ...
        //if mouse is outside base image
      ...
      var newpower =
        zoomdir == "in"
          ? Math.min(power + 1, powerrange[1])
          : Math.max(power - 1, powerrange[0]); //get new power from zooming in or out
  ...
  setupimage: function ($, imgref, options) {
    ...
    $imgref.unbind("mouseenter").mouseenter(function (e) {
      //mouseenter event over base image
      var $magnifier = ddpowerzoomer.$magnifier;
      $magnifier.outer.css({
      ...
      $magnifier.image = $magnifier.outer
        .find("img:first")
        .css({
  ...
  init: function ($) {
    var $magnifier = $(
      '<div style="border-radius:8px;position:absolute;width:100px;height:100px;display:none;overflow:hidden;border:1px solid black;z-index:1000"/>'
    )
    ...
    $(document)
      .unbind("mousemove.trackmagnifier")
      .bind("mousemove.trackmagnifier", function (e) {
        //bind mousemove event to doc
        if (ddpowerzoomer.activeimage) {
          //if mouse is currently over a magnifying image
...
jQuery.fn.addpowerzoom = function (options) {
  ...
  return this.each(function () {
    ...
    if (options.largeimage && options.largeimage.length > 0) {
      //preload large image?
      options.preloadimg = new Image();
      options.preloadimg.src = options.largeimage;
    }
    var $imgref = $(this);
    options.largeimagesrc = options.preloadimg
      ? options.preloadimg.src
      : $imgref.attr("src");
    if (parseInt(this.style.width) > 0 && parseInt(this.style.height) > 0)
      //if image has explicit CSS width/height defined
